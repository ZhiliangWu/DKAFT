...
from model_utils import VSequenceFeature, EpochOutputStore, CheckPointAfter, \
    get_initial_inducing_points_fasching, DKLModel, create_dkl_trainer,\
    create_dkl_evaluator
...
def run(batch_size=64, lr=3e-4, alpha=1e-3, n_hidden_sta=4, n_hidden_temp=32,
    ...
    inducing_points = get_initial_inducing_points_fasching(feature_extractor,
                                                           train_loader,
                                                           device,
                                                           num_inducing)
    # inducing_points = torch.randn(int(num_gp * num_inducing * batch_size),
    #                               n_hidden_sta + n_hidden_temp)
    ...
    prepare_batch_tensor_pfs_reshape = partial(prepare_batch_tensor_pfs,
                                               new_shape=(-1, ))
    trainer = create_dkl_trainer(model, likelihood, optimizer, mll,
                                 device=device,
                                 prepare_batch=prepare_batch_tensor_pfs_reshape,
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        handler = CheckPointAfter(start_epoch=30,
                                  to_save=to_save,
        ...
        def final_evaluation(engine):
            ...
            for batch in test_loader:
                with torch.no_grad():
                    x, y = prepare_batch_tensor_pfs(batch, device=device,
                                                    non_blocking=False,
                                                    new_shape=(-1,)
                                                    )
                    output = model_bk(x)
...
def get_exp_number_from_command_line():
    parser = argparse.ArgumentParser()
    parser.add_argument('-n', '--number', type=int, dest='number', default=0,
                        help='number for the experiment')
