...
from model_utils import SequenceFeature, LinearModel, EpochOutputStore, \
    CheckPointAfter


def lr_finder(batch_size, alpha, end_lr=10, diverge_th=5, n_hidden_sta=4,
              n_hidden_temp=32, model_name='lstm', n_embedding_temp=32,
              len_int=1, len_ts=24, fold_idx=0, device=torch.device('cpu'),
              exp_name='dataset_xxx', run_name='model_lrf', seed=42):
    ...
    backbone = SequenceFeature(n_feature_sta, n_feature_temp,
...
if __name__ == '__main__':
    ...
    #           len_ts=length_time, fold_idx=fold, device=dc, exp_name=exp,
    ...
    #         for n_hidden_t in [16, 32, 64]:
    #             for n_embedding_temp in [4, 8, 16, 32]:
    #                 run(batch_size=bs, lr=lrt, alpha=a,
    #                     n_hidden_sta=n_hidden_s,
    #                     n_hidden_temp=n_hidden_t, model_name=m_name,
    #                     n_embedding_temp=n_embedding_temp,
    #                     len_int=interval, len_ts=length_time,
    #                     epoch=n_epoch, fold_idx=fold, device=dc,
