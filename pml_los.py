...
from model_utils import SequenceFeature, create_metric_trainer


def lrfinder(batch_size, alpha, end_lr=10, diverge_th=5, n_hidden_sta=4,
             n_hidden_temp=32, model_name='lstm', n_embedding_temp=32,
             len_int=1, len_ts=48, fold_idx=0, exp_name='dataset_xxx',
             run_name='xmodel_metric_lrf', device='cpu', seed=42):
    ...
    dp = Path(f'/home/wu_z/gp_new/mimic_los/{exp_name[-4:]}')
    fn = f'imputed-normed-ep_{len_int}_{len_ts}_los_c.data'

    data_fp = dp / fn
    split_df = pd.read_csv(dp / f'{fn[:-7]}_split.csv', index_col=0)
    ...
    model = SequenceFeature(n_feature_sta, n_feature_temp,
    ...
    prepare_batch_c = partial(prepare_batch_tensor, new_shape=(-1,))
...
def run_metric_learning(batch_size=64, lr=3e-4, alpha=1e-3, n_hidden_sta=4,
                        ...
                        n_embedding_temp=32, len_int=1, len_ts=48, epoch=1,
                        fold_idx=0, device='cpu', exp_name='dataset_xxx',
                        run_name='model_metric', seed=42):
        ...
        def evaluate_model(engine):
            class BaseTesterNew(testers.BaseTester):
                def get_embeddings_for_eval(self, trunk_model, embedder_model,
                                            input_imgs):
                    static, temp_tensor = input_imgs
                    sta = static.to(self.data_device)
                    temp = temp_tensor.to(self.data_device)
                    trunk_output = trunk_model((sta, temp))
            ...
            data_and_label_getter = lambda x: ((x[0], x[1]), x[2])
...
if __name__ == '__main__':
    ...
    #          len_int=interval, len_ts=length_time, fold_idx=0,
    ...
    for f in range(1):
        _ = run_metric_learning(batch_size=bs, lr=lrt, alpha=a,
                                ...
                                n_embedding_temp=n_embedding_temp,
                                epoch=n_epoch, len_int=interval,
                                len_ts=length_time, fold_idx=f, device=dc,
